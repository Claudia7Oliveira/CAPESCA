      ...
      <div class="col-md-12"> 
        <div class="card card-primary">
          <div class="card-header">
            <h3 class="card-title">Adicionar história</h3>
          </div>
            <div class="card-body">
            <?php
             echo validation_errors('<div class="alert alert-danger text-center">','</div>');
             echo form_open_multipart('admin/historia/inserir');
            ?>
                      ...
                      img{ 
                         ...
                         <?php
                          ...
                          foreach($publicacoes as $post){
                              $titulo= $post->titulo;
                              if($post->img == 1){
                              $fotopub = img("assets/frontend/img/historia/".md5($post->id).".jpg");
                              }else{
                              $fotopub= img("assets/frontend/img/semFoto2.png");
                              }
                              $data= postadoem($post->data);
                              $alterar= anchor(base_url('admin/historia/alterar/'.md5($post->id)),'<i class="fa fa-refresh fa-fw"></i> Alterar');
                              $excluir= anchor(base_url('admin/historia/excluir/'.md5($post->id)),'<i class="fa fa-remove fa-fw"></i> Excluir');
