...
class Homesite extends MY_Controller {
	private $base_url;

	public function __construct(){
		parent::__construct();
		
		$this->load->model('homesite_model', 'model');

		$this->base_url = base_url("admin/homesite");
	}
	...
	public function index($pular = null, $post_por_pagina = 5)
	{
		...
		$this->load->view('backend/homesite');
	...
	private function upload_image($id)
	{
		$config = [
			'upload' => [
				'upload_path' => './assets/frontend/img/home',
				'allowed_types' => 'jpg',
			...
			'image' => [
				...
				'height' => 300,
				'create_thumb' => FALSE,
				'source_image' => './assets/frontend/img/home/'.md5($id).'.jpg'
			]
		];
	...
	public function excluir($id){
		if($this->model->excluir($id)){
				redirect(base_url('admin/homesite'));
			}else{
				echo "Houve um erro no sistema!";
			}
	}
	public function alterar($id){
		...
		$this->load->view('backend/template/header');
		$this->load->view('backend/alterar_homesite');
		$this->load->view('backend/template/footer');
		$this->load->view('backend/template/html-footer');
	}
	...
	public function nova_foto(){
		...
		$config['upload_path']= './assets/frontend/img/home';
		...
		if(!$this->upload->do_upload()){
			...
			$config2['source_image']= './assets/frontend/img/home/'.$id.'.jpg';
			...
			if($this->image_lib->resize()){
				...
				redirect(base_url('admin/homesite/alterar/'.$id));
				}else{
					echo "Houve um erro no sistema!";
				}
