...
class Noticia extends MY_Controller {
	...
	public function __construct(){
		...
		$this->load->model('noticia_model', 'model');

		$this->base_url = base_url("admin/noticia");
	}
	...
	public function index($pular = null, $post_por_pagina = 5)
	{
		...
		$this->load->view('backend/noticia');
	...
	private function upload_image($id)
	{
		$config = [
			'upload' => [
				'upload_path' => './assets/frontend/img/noticia',
				'allowed_types' => 'jpg',
			...
			'image' => [
				...
				'height' => 300,
				'create_thumb' => FALSE,
				'source_image' => './assets/frontend/img/noticia/'.md5($id).'.jpg'
			]
		];
	...
	public function alterar($id){
		...
		$this->load->view('backend/template/header');
		$this->load->view('backend/alterar_noticia');
		$this->load->view('backend/template/footer');
		$this->load->view('backend/template/html-footer');
	}
