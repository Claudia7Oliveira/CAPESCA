...
class Historia extends MY_Controller {
	...
	public function __construct(){
		...
		$this->load->model('historia_model', 'model');

		$this->base_url = base_url("admin/historia");
	}
	...
	public function index($pular = null, $post_por_pagina = 5)
	{
		...
		$this->load->view('backend/historia');
	...
	private function upload_image($id)
	{
		$config = [
			'upload' => [
				'upload_path' => './assets/frontend/img/historia',
				'allowed_types' => 'jpg',
			...
			'image' => [
				...
				'height' => 300,
				'create_thumb' => FALSE,
				'source_image' => './assets/frontend/img/historia/'.md5($id).'.jpg'
			]
		];
	...
	private function validate()
	{
		...
		$this->form_validation->set_rules('txt-subtitulo', 'Subtitulo', 'required|min_length[20]');
	...
	public function alterar($id){
		...
		$this->load->view('backend/template/header');
		$this->load->view('backend/alterar_historia');
		$this->load->view('backend/template/footer');
		$this->load->view('backend/template/html-footer');
	}
	public function salvar_alteracoes($idCrip){
		...
		if ($this->form_validation->run() == FALSE){
			...
			if($this->model->alterar($titulo,$subtitulo,$conteudo,$datapub,$id)){
				redirect(base_url('admin/historia'));
			}else{
				echo "Houve um erro no sistema!";
			}
	...
	public function nova_foto(){
		...
		$config['upload_path']= './assets/frontend/img/historia';
		...
		if(!$this->upload->do_upload()){
			...
			$config2['source_image']= './assets/frontend/img/historia/'.$id.'.jpg';
			...
			if($this->image_lib->resize()){
				...
				redirect(base_url('admin/historia/alterar/'.$id));
				}else{
					echo "Houve um erro no sistema!";
				}
